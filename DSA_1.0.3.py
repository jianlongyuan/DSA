...
For more details please refer to our paper below:
  ...
  Journal of Geophysical Research: Solid Earth, 125(7), e2020JB019430.
...
Improvement at:
  ...
  2022.08.10
    Suggestion from Ackerley Nicholas:
      Support loading: QuakeML and miniSEED.
  2022.08.11
...
#%%-- check waveform files in sac or miniSEED format
sacFiles   = fnmatch.filter( sorted(os.listdir(dataPath)), '*.sac')
mseedFiles = fnmatch.filter( sorted(os.listdir(dataPath)), '*.mseed')

if len(sacFiles) > 0:
    wfFiles = sacFiles
    wfFormat= 'sac'
elif len(mseedFiles) > 0:
    wfFiles = mseedFiles
    wfFormat= 'mseed'
else: sys.exit('Can not find waveforms in sac or miniSEED format!')
...
for ist in range( numSt ):    
    ...
    if wfFormat == 'sac':
        evla   = stRawZ[0].stats.sac.evla
        evlo   = stRawZ[0].stats.sac.evlo
        stla   = stRawZ[0].stats.sac.stla
        stlo   = stRawZ[0].stats.sac.stlo
        ...
        st_begin_time = stRawZ[0].stats.sac.b # used for fk synthetic data
        st_offset_time= stRawZ[0].stats.sac.o
        ev_onset_timeUTC = UTCDateTime(stRawZ[0].stats.starttime+st_offset_time)
    elif wfFormat == 'mseed':
        evPars = read_events(dataPath+"eventParameters.xml")
        inv    = read_inventory( "{0}{1}.{2}.xml".format( dataPath, stRawZ[0].stats.network, stRawZ[0].stats.station ) )
        evtime = evPars[0].origins[0].time
        evla   = evPars[0].origins[0].latitude
        evlo   = evPars[0].origins[0].longitude
        evdp   = evPars[0].origins[0].depth /1000.0
        ...
        recDisInDeg = kilometer2degrees(recDisInKm)
        epiDisEachStation[ist] = recDisInDeg
        nameEachStation[ist].append( stRawZ[0].stats.station.strip() )
        azimuthEachStation[ist] = azimuth
        DT = stRawZ[0].stats.delta
        st_begin_time = 0 # used for fk synthetic data
        st_offset_time= 0
        ev_onset_timeUTC = UTCDateTime(stRawZ[0].stats.starttime+st_offset_time)
    ...
    print(infileN.name)
    print(infileZ.name)
    print("Station = ", nameEachStation[ist] )
    print("Scanning station:", ist+1, '/', numSt)
    print("evla, evlo = ", evla, evlo  )
    print("stla, stlo = ", stla, stlo  )
    print("Az, Baz    = ", azimuthEachStation[ist], baz )
    print("recDisInKm = ", recDisInKm)
    print("recDisInDeg= ", recDisInDeg)
    print("Dt = ", DT)
    print("starttime  = ", stRawZ[0].stats.starttime)
    print("st_offset_time   = ", st_offset_time)
    print("ev_onset_timeUTC = ", ev_onset_timeUTC)
    print("--------------------------------------------------------\n")
    ...
    '''
    if azimuthEachStation[ist] < 180 or azimuthEachStation[ist] >= 270:
           continue
    '''      
    
    #%%-- Extract wavefroms within wanted time window
    stWantedE = stRawE.trim(ev_onset_timeUTC, ev_onset_timeUTC+wantedTimeLengZ)
    stWantedN = stRawN.trim(ev_onset_timeUTC, ev_onset_timeUTC+wantedTimeLengZ)
    stWantedZ = stRawZ.trim(ev_onset_timeUTC, ev_onset_timeUTC+wantedTimeLengZ)
...
#        print(stT0)    
    ...
    if np.fabs( onsetZNor+st_begin_time-calOnsetP ) > 5 or\
       np.fabs( onsetRNor+st_begin_time-calOnsetP ) > 5 or\
       np.fabs( onsetTNor+st_begin_time-calOnsetS ) > 5:
        print("\n Onset time match failed, skip station=", nameEachStation[ist], 
    ...
    if verboseFlag == 1:
        ...
        tKur = np.arange( 0, len(kurtosisZ), 1)*DT+st_begin_time
        tT0  = np.arange( 0, len(stNorZ0), 1)*DT+st_begin_time
        ...
        pltDebug.scatter( onsetZ+st_begin_time, 0.25, marker="o", s=100, label='Picked onset',
        ...
        tKur = np.arange( 0, len(kurtosisR), 1)*DT+st_begin_time
        tT0  = np.arange( 0, len(stNorR0), 1)*DT+st_begin_time
        pltDebug.figure(figsize=(12,2))
        pltDebug.tick_params(axis='both', which='major', labelsize=10)
        pltDebug.xlabel('Time (s)', fontsize=12)
        pltDebug.ylabel('Normalized Amp.', fontsize=12)
        ...
        pltDebug.scatter( onsetR+st_begin_time, 0.25, marker="o", s=100, label='Picked onset',
                          facecolor='none', edgecolor='black', lw=1, zorder=101 )
        pltDebug.scatter( calOnsetP, 0.5, marker="o", s=100, label='Theoretical onset', 
                          facecolor='none', edgecolor='red', lw=1, zorder=101 )
        pltDebug.margins(0)
        ...
        tKur = np.arange( 0, len(kurtosisT), 1)*DT+scanBegTimeT+st_begin_time
        tT0  = np.arange( 0, len(stNorT0), 1)*DT+st_begin_time
        pltDebug.figure(figsize=(12,2))
        pltDebug.tick_params(axis='both', which='major', labelsize=10)
        pltDebug.xlabel('Time (s)', fontsize=12)
        pltDebug.ylabel('Normalized Amp.', fontsize=12)
        ...
        pltDebug.scatter( onsetT+st_begin_time, 0.25, marker="o", s=100, label='Picked onset',
                          facecolor='none', edgecolor='black', lw=1, zorder=101 )
    ...
    if verboseFlag == 1:       
        ...
        t = np.arange( 0, len( templateZ[0] ), 1 )*DT+temZtBegNor+st_begin_time
        ...
        pltDebug.figure( figsize=(5,2))
        t = np.arange( 0, len( templateR[0] ), 1 )*DT+temRtBegNor+st_begin_time
        pltDebug.axhline( 0, linewidth=0.5, color='gray' )
        ...
        pltDebug.figure( figsize=(5,2) )
        t = np.arange( 0, len( templateT[0] ), 1 )*DT+temTtBegNor+st_begin_time
        pltDebug.axhline( 0, linewidth=0.5, color='gray' )
    ...
    for phaseShift in range(phaseShiftStart, phaseShiftEnd, PhaseShittInc):
        ...
        temp = hilbert(templateZ[0])
        temp = np.real(np.abs(temp) * np.exp((np.angle(temp) +\
                        (phaseShift)/180.0 * np.pi) * 1j))      
        #-- cross-corelation
        corrValZ[count] = xcorrssl( scanTimeBegP, scanTimeEndP, temp, np.array(stZ0[0]))
        count += 1
    ...
    for phaseShift in range(phaseShiftStart, phaseShiftEnd, PhaseShittInc):
        #-- Phase shift using Hilbert transform
        temp = hilbert(templateR[0])
        temp = np.real(np.abs(temp) * np.exp((np.angle(temp) +\
                        (phaseShift)/180.0 * np.pi) * 1j))          
        #-- cross-corelation
        corrValR[count] = xcorrssl( scanTimeBegP, scanTimeEndP, temp, np.array(stR0[0]))
        count += 1
    ...
    for phaseShift in range(phaseShiftStart, phaseShiftEnd, PhaseShittInc):
        #-- Phase shift using Hilbert transform
        temp = hilbert(templateT[0])
        temp = np.real(np.abs(temp) * np.exp((np.angle(temp) +\
                        (phaseShift)/180.0 * np.pi) * 1j))          
        #-- cross-corelation
        corrValT[count] = xcorrssl( scanTimeBegS, scanTimeEndS, temp, np.array(stT0[0]))
        count += 1
...
ax11.scatter(prelimSolution, sumAvgArrTimeDiffResGlobal[int(prelimSolution)-1],
...
if len( wellBehavedStationId ) < 1:
    ...
    for isw in range( numWellBehavedStation ):
        ...
        if plotSteps1n2Flag == 1:
            ...
            # set span
            ax0.axhspan(leftBoundry1Z, rightBoundry1Z, facecolor='0.5', alpha=0.5)
            ax5.axhspan(leftBoundry1Z, rightBoundry1Z, facecolor='0.5', alpha=0.5)
            ...
            # set span
            ax0.axhspan(leftBoundry1R, rightBoundry1R, facecolor='0.5', alpha=0.5)
            ax5.axhspan(leftBoundry1R, rightBoundry1R, facecolor='0.5', alpha=0.5)
            ...
            # set span
            ax0.axhspan(leftBoundry1T, rightBoundry1T, facecolor='0.5', alpha=0.5)
            ax5.axhspan(leftBoundry1T, rightBoundry1T, facecolor='0.5', alpha=0.5)
